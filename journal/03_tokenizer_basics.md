...
Tokenizers generally work at one of three levels: word level, where each token is a whole word; sentence level, where each token is a full sentence; or subword level, where each token is a piece somewhere in between, smaller than a word but usually bigger than a single character. Word level tokenizers struggle with a language like Tamil, since new words get built constantly through agglutination, so the vocabulary would need to be enormous to cover everything. Subword tokenizers avoid that problem by learning common pieces instead of whole words, so a word the tokenizer has never seen before can still be represented as a combination of familiar pieces. This project uses subword tokenizers throughout. 
