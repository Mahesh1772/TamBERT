"""
Entry point for the BPE family of the tokenizer ablation study.
Shared training logic lives in pipeline.py; this just points it at
this folder's config.yaml.

    python train_bpe_tokenizer.py
    python train_bpe_tokenizer.py --only 03_sandhi_codepoint_bpe
"""
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Train BPE tokenizer variants")
    parser.add_argument('--config', default=str(Path(__file__).resolve().parent / 'config.yaml'))
    parser.add_argument('--only', help="Train a single variant by name, e.g. 03_sandhi_codepoint_bpe")
