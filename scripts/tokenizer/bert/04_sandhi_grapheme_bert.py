...
sandhi_grapheme_bert = Tokenizer(WordPiece())

# Normalizer
sandhi_grapheme_bert.normalizer = normalizers.Sequence([normalizers.NFC(),
...
sandhi_grapheme_bert.pre_tokenizer = pre_tokenizers.Metaspace()
...
print(sandhi_grapheme_bert.pre_tokenizer.pre_tokenize_str(sample_substituted))

# Trainer
sandhi_grapheme_bert_trainer = WordPieceTrainer(
...
sandhi_grapheme_bert.train([str(paths.train_sandhi_grapheme_marked)], trainer=sandhi_grapheme_bert_trainer)
...
sandhi_grapheme_bert.decoder = decoders.Sequence([decoders.WordPiece(prefix="##", cleanup=True),
                                        decoders.Metaspace(),
                                        decoders.Replace(Regex("⟂"), "")])

# Evaluate
train_metrics = calculate_tokenizer_metrics(sandhi_grapheme_bert, paths.test_sandhi_grapheme_marked)

# Post-processor — BERT [CLS]/[SEP] structure, set after training since it
# needs real token IDs from the trained vocab
sandhi_grapheme_bert.post_processor = processors.TemplateProcessing(
    single="[CLS] $A [SEP]",
    pair="[CLS] $A [SEP] $B:1 [SEP]:1",
    special_tokens=[
        ("[CLS]", sandhi_grapheme_bert.token_to_id("[CLS]")),
        ("[SEP]", sandhi_grapheme_bert.token_to_id("[SEP]")),
    ],
)

# Report
print(f"Training completed in {cpu_time_taken:.2f}s (CPU) / {wall_time_taken:.2f}s (wall).")
print(f"Vocabulary size: {len(sandhi_grapheme_bert.get_vocab())}")
print(f"Fertility on test data: {train_metrics['fertility']:.4f}")
print(f"OOV rate on test data: {train_metrics['oov_rate']:.4f}")

encoded = sandhi_grapheme_bert.encode(sample_substituted)
print(f"Encoding on placeholder text (with [CLS]/[SEP]): {encoded.tokens}")

decoded_placeholder = sandhi_grapheme_bert.decode(encoded.ids)
print(f"Decoded (restored to raw Tamil): {restore_text(decoded_placeholder, placeholder_to_grapheme)!r}")

# Save
out_dir = paths.tokenizer_name_generator('04_sandhi_grapheme_bert')
sandhi_grapheme_bert.save(str(out_dir / 'tokenizer.json'))
...
with open(out_dir / 'metrics.json', 'w', encoding='utf-8') as f:
    json.dump({
        ...
        'vocab_size': len(sandhi_grapheme_bert.get_vocab()),
