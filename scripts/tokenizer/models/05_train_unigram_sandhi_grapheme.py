...
print(grapheme_unigram_sandhi.pre_tokenizer.pre_tokenize_str(SAMPLE_TEXT))
...
grapheme_unigram_sandhi.train([str(paths.train_sandhi_marked)], trainer=grapheme_unigram_sandhi_trainer)
...
# Evaluate
train_metrics = calculate_tokenizer_metrics(grapheme_unigram_sandhi, paths.test_sandhi_marked)
...
encoded = grapheme_unigram_sandhi.encode(SAMPLE_TEXT)
print(f"Encoding (with [CLS]/[SEP]): {encoded.tokens}")
print(f"Decoded: {grapheme_unigram_sandhi.decode(encoded.ids)!r}")

# Save
out_dir = paths.tokenizer_name_generator('unigram_grapheme_sandhi')
